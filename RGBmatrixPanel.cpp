/*
Modified by Russell Peake to use faster scan on solid colour;
...
#include "EnhancedRGBmatrixPanel.h"
#include "gamma.h"
...
static EnhancedRGBmatrixPanel *activePanel = NULL;

// Code common to both the 16x32 and 32x32 constructors:
void EnhancedRGBmatrixPanel::init(uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
...
EnhancedRGBmatrixPanel::EnhancedRGBmatrixPanel(
...
void EnhancedRGBmatrixPanel::begin(void) {
...
uint16_t EnhancedRGBmatrixPanel::Color333(uint8_t r, uint8_t g, uint8_t b) {
  // RRRrrGGGgggBBBbb
...
uint16_t EnhancedRGBmatrixPanel::Color444(uint8_t r, uint8_t g, uint8_t b) {
  // RRRRrGGGGggBBBBb
...
uint16_t EnhancedRGBmatrixPanel::Color888(uint8_t r, uint8_t g, uint8_t b) {
  return ((r & 0xF8) << 11) | ((g & 0xFC) << 5) | (b >> 3);
}

// 8/8/8 -> gamma -> 5/6/5
uint16_t EnhancedRGBmatrixPanel::Color888(
...
uint16_t EnhancedRGBmatrixPanel::ColorHSV(
...
void EnhancedRGBmatrixPanel::drawPixel(int16_t x, int16_t y, uint16_t c) {
...
void EnhancedRGBmatrixPanel::fillScreen(uint16_t c) {
...
uint8_t *EnhancedRGBmatrixPanel::backBuffer() {
...
void EnhancedRGBmatrixPanel::swapBuffers(boolean copy) {
...
void EnhancedRGBmatrixPanel::dumpMatrix(void) {
...
ISR(TIMER1_OVF_vect, ISR_BLOCK) { // ISR_BLOCK important -- see notes later
  if (activePanel->useStatic > 0) {
    activePanel->updateStaticDisplay();
  } else {
...
void EnhancedRGBmatrixPanel::updateDisplay(void) {
...
//unset static flag
void EnhancedRGBmatrixPanel::setDynamic() {
 useStatic = 0; 
}

//set colour for static display
void EnhancedRGBmatrixPanel::setColour(byte col) {
 if (col >= 0 & col < 8) {
   if (useStatic == 0)
     currentColour = -1;
   useStatic = 1;
...
//displays a static colour at a much faster scan rate
void EnhancedRGBmatrixPanel::updateStaticDisplay(void) {
  uint8_t  i, tick, tock, *ptr;
  uint16_t t, duration;

  *oeport  |= oepin;  // Disable LED output during row switchover
  *latport |= latpin; // Latch data loaded during *prior* interrupt, could potentially be moved into colour update section
  
  //save appropriate value togle pins to speed up clock writting
  tock = SCLKPORT;
  tick = tock | sclkpin;
  
  if(++row >= nRows) {        // advance row counter.  Maxed out?
      row     = 0;
      if (currentColour != nextColour) { //new colour registered?
        currentColour = nextColour; //update current colour to reflect change
        for (i=0; i<32; i++) { //write out 32 LED's worth of colour values
          DATAPORT = (currentColour << 2) | (currentColour << 5); //(rgb lines connected to pins 7-2 on dataport
          SCLKPORT = tick; // Clock lo
          SCLKPORT = tock; // Clock hi
        }
      }   
  }
 //always advance the address pin as each row is written once
 if(row & 0x1)   *addraport |=  addrapin;
  else            *addraport &= ~addrapin;
 if(row & 0x2)   *addrbport |=  addrbpin;
  else            *addrbport &= ~addrbpin;
 ...
 if(nRows > 8) {
  ...
  ICR1      = 100; // Set interval for next interrupt (smaller than this and time in interupt vs time in listen loop becomes an issue
  TCNT1     = 0;        // Restart interrupt timer
  if (currentColour != 0) //HACKL: writting all zeros to the dataport results in non-stable colour on the leds, leaving the panel off is a simple fix
  *oeport  &= ~oepin;   // Re-enable output
  *latport &= ~latpin;  // Latch down
}
